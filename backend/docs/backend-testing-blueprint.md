...
| Patient registration code issue under `POST /patient-app/{patient_id}/code` | Strong | `test_api.py` and `test_patient_app_auth.py` cover assigned-doctor access, admin success, staff denial, previous-code invalidation, and not-found behavior | add audit coverage only if the endpoint later becomes an audited support workflow |
...
| Alerts acknowledge under `/alerts/{alert_id}/acknowledge` | Strong | `test_dense_mode_access.py` covers assigned doctor, unassigned doctor, admin, and staff | add malformed payload validation only if schema changes |
