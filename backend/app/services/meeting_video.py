    ...
    *,
    ...
    for _ in range(PATIENT_INVITE_SHORT_CODE_MAX_RETRIES):
        ...
        try:
            ...
            _cache_short_code_record(short_link)
            return short_link
...
def get_active_patient_invite_code(
    *,
    ...
    cached_invite_code = _load_cached_active_short_code(meeting_id=meeting_id)
    if cached_invite_code is not None:
        return cached_invite_code
    ...
    for invite_code in invite_codes:
        if _as_utc(invite_code.expires_at) > now:
            _cache_short_code_record(invite_code)
            return invite_code
        _clear_short_code_cache(short_code=invite_code.code)
    return None
...
def _resolve_patient_short_code_record(
    *,
    ...
    cached_short_link = _load_cached_short_code_record(short_code=normalized)
    if cached_short_link is not None:
        return cached_short_link
    ...
    if expires_at <= now:
        _clear_short_code_cache(short_code=normalized, meeting_id=short_link.meeting_id)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    *,
    ...
    for invite_code in invite_codes:
        if _as_utc(invite_code.expires_at) > now:
            ...
            db.add(invite_code)
        _clear_short_code_cache(short_code=invite_code.code, meeting_id=meeting.id)
...
def create_patient_join_invite(
    *,
    ...
    if active_short_link:
        ...
        _cache_short_code_record(active_short_link)
