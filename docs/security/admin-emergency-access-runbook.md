...
- `POST /security/admin-unlock` for audited admin unlock by `super-admin`
  users,
- `backend/scripts/emergency_unlock_admin.py` for CLI-based emergency unlock,
- `super-admin` password reset and 2FA reset controls in the security toolkit,
...
4. Use a `super-admin` account if one is still available.
...
Use the built-in security tools whenever at least one `super-admin` can still
sign in.
...
no trusted `super-admin` session is active.
...
- matching `--requester-email` against `SUPER_ADMIN_EMAILS`, or
