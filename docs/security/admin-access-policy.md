...
In the current implementation, every `super-admin` is an `admin` account whose
email is listed in `SUPER_ADMIN_EMAILS`.
...
- `super-admin` permissions are assigned by membership in
  `SUPER_ADMIN_EMAILS`.
- New `admin` accounts must be created through invite flow.
- Only `super-admin` accounts may issue or resend `admin` invites.

This repository does not implement a permanent application-level
"break-glass user" account. Emergency recovery uses controlled
`super-admin` actions and the dedicated emergency unlock tooling.
...
- backup codes,
- trusted devices, and
- super-admin 2FA reset flows.

## Onboarding policy

Admin onboarding must follow this sequence:

1. A `super-admin` creates an `admin` invite.
...
5. The `super-admin` verifies the new admin can sign in and reach the required
   areas.
...
1. Remove the account from `SUPER_ADMIN_EMAILS` if applicable.
2. Disable or delete the user account according to your HR and audit policy.
3. Revoke trusted devices.
4. Rotate any shared recovery material associated with that operator.
5. Review audit logs for the last 30 days.
...
- normal recovery uses `super-admin` tooling,
